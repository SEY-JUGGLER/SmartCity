<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-950 dark:to-gray-900">
    ...
    <main class="max-w-3xl mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <button wire:click="marquerToutesLues" class="text-sm text-orange-600">Tout marquer lu</button>
        </div>
        <div class="space-y-2">
            @forelse($notifications as $n)
                <div class="p-4 rounded-xl border {{ $n->read_at ? 'bg-slate-50 dark:bg-gray-800/50' : 'bg-white dark:bg-gray-900 border-orange-200' }}">
                    ...
                    @unless($n->read_at)
                        <button wire:click="marquerLue('{{ $n->id }}')" class="text-xs text-orange-600 mt-2">Marquer comme lu</button>
                    @endunless
                </div>
            @empty
